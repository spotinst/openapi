...
.PHONY: bundle
bundle: ## Make a static HTML file with ReDoc
	docker run --rm -it -v $(PWD):/src -w /src node:14 npx redoc-cli bundle --output build/index.html api/spot.yaml

.PHONY: bundle-json
bundle-json: ## Make a single JSON bundle of the OpenAPI spec
	docker run --rm -it -v $(PWD):/src -w /src node:14 npx swagger-cli bundle --outfile build/swagger.json api/spot.yaml
